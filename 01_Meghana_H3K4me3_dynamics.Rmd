...
if(clustered_chip_groups){
  chip_clustering <- list(Kept = "Y_Y_Y_Y", Recovered=c("Y_Y_N_Y", "Y_N_Y_Y", "Y_N_N_Y"), Lost=c("Y_Y_Y_N", "Y_Y_N_N", "Y_N_N_N"), Gained=c("N_N_Y_Y", "N_N_N_Y"), Absent="N_N_N_N")
  cols_clustered = setNames(c("#DDAA33", "#228833","#BB5566", "#004488", "#DDDDDD"), names(chip_clustering))
} else {
  chip_clustering <- list("Kept" = "Y_Y_Y_Y", "RecoveredLate"="Y_Y_N_Y", "RecoveredEarly"="Y_N_Y_Y", "RecoveredLong"="Y_N_N_Y", "Lost@ZGA"="Y_Y_Y_N", "Lost@Pre-ZGA"="Y_Y_N_N", "Lost@Sperm"="Y_N_N_N", "GainedEarly"="N_N_Y_Y", "GainedLate"="N_N_N_Y", "Absent"="N_N_N_N")
  cols_clustered = setNames(c("#DDCC77","#009988", "#44BB99", "#228833","#CC6677","#882255","#AA4499","#332288", "#88CCEE","#DDDDDD"), names(chip_clustering))
}

#Blood contimation GO plots
#chip_clustering <- list(N_Y_ = c("N_Y_Y_Y", "N_Y_Y_N", "N_Y_N_Y", "N_Y_N_N"))

#All groups
#chip_clustering <- setNames(list(Y_Y_Y_Y, Y_Y_Y_N, Y_Y_N_Y, Y_Y_N_N, Y_N_Y_Y, Y_N_Y_N, Y_N_N_Y, Y_N_N_N, N_Y_Y_Y, N_Y_Y_N, N_Y_N_Y, N_Y_N_N, N_N_Y_Y, N_N_Y_N, N_N_N_Y, N_N_N_N), chip_names_complete)
#cols_clustered <- setNames(c("#ece0d3","#a6afb7","#937c65","#625c5a","#be8162","#713725","#b73a2f","#e17e39","#f2c237","#6ca239","#36753d","#3b4ca8","#5f91c1","#6bc7ab","#d0589f","#8d55ae"), chip_names_complete)
...
chip_dyns <- lapply(chip_clustering, get_dyns)
...
pdf(file="chip_legend.pdf", width=10, height=10)
plot.new()
#plot(NULL, ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
legend("center", title="H3K4me3", bty = "y",
groups, fill=cols_chipdyns[groups], ncol = 1)
dev.off()
...
ggplot(data.frame("sizes"=chip_dyns_sizes, "dyn"=names(chip_dyns_sizes), "id"=1), aes(fill=dyn, y=sizes, x=id)) + 
    geom_bar(position="fill", stat="identity", fill=cols_chipdyns) + theme_pubr() + ylab("Ratio of genome") + theme(axis.title.x=element_blank(),
...
plot_alluvial <- function(remove_NNNN, remove_YYYY, equal_scale){
  ...
  #plot_df$Dynamic <- factor(plot_df$Dynamic, levels = c("Y_Y_Y_Y","Y_Y_Y_N","Y_Y_N_N", "Y_N_N_N", "N_Y_N_N", "N_N_N_Y", "Y_N_N_Y", "N_N_Y_Y",  "Y_Y_N_Y", "Y_N_Y_Y"))
  ...
  if(equal_scale){
    plot_df$Freq <- 1
    #for(stage in stages){
    #  plot_df[plot_df[,stage] == "Y","Freq"] <- plot_df[plot_df[,stage] == "Y","Freq"]/
    #    sum(plot_df[plot_df[,stage] == "Y","Freq"])
    #  plot_df[plot_df[,stage] == "N","Freq"] <- plot_df[plot_df[,stage] == "N","Freq"]/
    #    sum(plot_df[plot_df[,stage] == "N","Freq"])
    #}
  }else{
...
pdf(file="alluvial.pdf", width=8, height=5)
plot_alluvial(remove_NNNN = FALSE, remove_YYYY = FALSE, equal_scale=equal_scale)
dev.off()

plot_alluvial(remove_NNNN = TRUE, remove_YYYY = FALSE, equal_scale=equal_scale)
plot_alluvial(remove_NNNN = TRUE, remove_YYYY = TRUE, equal_scale=equal_scale)
...
g <- arrangeGrob(res_width[[1]], res_width[[2]], res_width[[3]], res_width[[4]], 
                 res_mean[[1]], res_mean[[2]], res_mean[[3]], res_mean[[4]],
                  ...
                  ncol=4)
ggsave(file=paste0("chip_stage_maintenance.pdf"), g, width = 25, height = 6)
```
