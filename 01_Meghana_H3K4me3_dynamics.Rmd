...
chip_clustering <- list(Kept = "Y_Y_Y_Y", Recovered="Y_Y_N_Y", Nost=c("Y_Y_Y_N", "Y_Y_N_N"), Gained=c("N_N_Y_Y", "N_N_N_Y"), Absent="N_N_N_N")
...
```{r fig.height=5, fig.width=10}
plot_stage_dynamics <- function(timepoint, type){
  ...
  violin_plot <- ggviolin(plot_df, x = "dyn", y = paste(type), fill = "dyn",
           add = "boxplot", add.params = list(fill = "white")) + scale_fill_manual(values=cols_chipdyns) + rotate_x_text(45)
  ecdf_plot <- ggecdf(plot_df, x = paste(type), color = "dyn") + scale_colour_manual(values=cols_chipdyns) 
  grid.arrange(violin_plot, ecdf_plot, top=paste(timepoint), ncol=2)
}

plot_stage_dynamics(timepoint = "Spermatid", type = "peak.width")
plot_stage_dynamics(timepoint = "Spermatid", type = "promoter.enrich")
plot_stage_dynamics(timepoint = "Spermatid", type = "promoter.mean")
plot_stage_dynamics(timepoint = "Spermatid", type = "promoter.max")

plot_stage_dynamics(timepoint = "ZGA", type = "peak.width")
plot_stage_dynamics(timepoint = "ZGA", type = "promoter.enrich")
plot_stage_dynamics(timepoint = "ZGA", type = "promoter.mean")
plot_stage_dynamics(timepoint = "ZGA", type = "promoter.max")
