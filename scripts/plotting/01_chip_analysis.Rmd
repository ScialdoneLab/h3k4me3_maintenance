...
ggsave('output/chip/genomic_distribution.pdf', width=5, height=5)
...
if (tolower(response) == "yes") {
  save(chip_dyns, chip_dyns_detail, chip_dyns_removed, file="chip_dynamics.RData")
} else {
...
legend("center", legend=groups, title="H3K4me3", bty = "y", fill=cols_chip_detail[groups], ncol = 1)
dev.off()
...
```{r}
for(stage in stages){
  g <- plot_stage_dynamics(chip_dfs, dyns=chip_dyns, cols=cols_chip, timepoint=stage, type="promoter.mean", only_violin = TRUE)
  grid.arrange(g)
...
g <- plot_stage_dynamics(chip_dfs, dyns=dyns, cols=cols, timepoint="Spermatid", type="promoter.mean", only_violin = TRUE)
grid.arrange(g)
ggsave(file=paste0("output/chip/Spermatid_violin_plot_detail.pdf"), g, width = 5, height = 4)

g <- plot_stage_dynamics(chip_dfs, dyns=dyns, cols=cols, timepoint="Spermatid", type="peak.width", only_violin = TRUE)
grid.arrange(g)
...
g <- plot_stage_dynamics(chip_dfs, dyns=dyns, cols=cols, timepoint="ZGA", type="promoter.mean", only_violin = TRUE)
grid.arrange(g)
ggsave(file=paste0("output/chip/ZGA_violin_plot_detail.pdf"), g, width = 5, height = 4)

g <- plot_stage_dynamics(chip_dfs, dyns=dyns, cols=cols, timepoint="ZGA", type="peak.width", only_violin = TRUE)
grid.arrange(g)
...
for (i in 1:length(dyns)){
  ...
  if(sum(chip_GO@result$p.adjust < chip_GO@pvalueCutoff)){
    pdf(file=paste0("output/chip/GO_", names(dyns)[i], ".pdf"), width=8, height=12)
...
pdf(file=paste0("output/chip/GO_summary_chip_dynamics.pdf"), width=10, height=6)
