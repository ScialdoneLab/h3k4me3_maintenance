...
if (tolower(response) == "yes") {
  if(clustered_rna_groups){
    save(rna_dyns, cols_rna, expression.data, file="rna_data_clustered.RData")
  } else {
    save(rna_dyns, cols_rna, expression.data, file="rna_data.RData")
  }
} else {
...
pdf("output/rna/rna_dynamics_heatmap.pdf", width=6, height=8)
draw(h1)
dev.off()
```
```{r, fig.width=6, fig.height=8}
...
col_split = factor(c(rep("Spermatid", 1), rep("Egg", 1), rep("ZGA", 4)))
h1 <- Heatmap(hm_values.log, row_split = tpms_ct_plot$Timing, name="log2(TPM+1)",
               cluster_columns = FALSE, cluster_rows = FALSE, show_row_names = FALSE, column_split = col_split, row_title_rot = 0, cluster_row_slices = FALSE,
...
## Balloon plot
```{r}
plot_balloonplot(x_dyns=rna_dyns_timing, y_dyns=rna_dyns, limit=200, remove_dyns=c("ND", "NonZygotic"), x_caption="ZGA Timing", y_caption="RNA dynamics")
ggsave('output/rna/rna_timing_balloon_plot.pdf', width=5, height=3)
```

## Violin / ECDF plot
```{r}
data <- list(ZGA = log2(as.data.frame(expression.data, row.names=allgenes$gene_id) + 1))
expression_plot <- plot_stage_dynamics(data, dyns=rna_dyns_timing, cols=cols_rna_timing, timepoint="ZGA", type="X9hpf")
grid.arrange(expression_plot)
ggsave('output/rna/rna_timing_9hpf_violin_ecdf.pdf', expression_plot, width=10, height=5)
```
...
pdf(file="output/rna/GO_summary_rna_dynamics.pdf", width=10, height=6)
...
go_list <- go_list[unlist(lapply(go_list, function(enrichGO){sum(enrichGO@result$p.adjust<p_adj) > 0}))]
go_list <- lapply(go_list, function(x) x$ID[x$p.adjust < p_adj])
pdf(file="output/rna/GO_summary_rna_timing_dynamics.pdf", width=10, height=6)
simplifyGOFromMultipleLists(go_list, padj_cutoff=p_adj, db = 'org.Xl.eg.db', ont = "BP")
dev.off()
simplifyGOFromMultipleLists(go_list, padj_cutoff=p_adj, db = 'org.Xl.eg.db', ont = "BP")
```
