...
plot_chip_diffs <- function(groups, type){
  ...
  #ggviolin(plot_df, x = "dyn", y = paste(type), fill = "dyn") +
  #        # add = "boxplot", add.params = list(fill = "white")) + 
  #  facet_wrap(~Stage) + scale_fill_manual(values=cols) #+ 
    #geom_signif(comparisons = split(t(combn(c(names(dyn), "NA"), 2)), seq(nrow(t(combn(c(names(dyn), "NA"), 2))))), 
