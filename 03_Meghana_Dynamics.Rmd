...
plot_stage_dynamics <- function(timepoint, type, groups, onlypeaks=FALSE){
  ...
  violin_plot <- ggviolin(plot_df, x = "dyn", y = paste(type), fill = "dyn",
           add = "boxplot", add.params = list(fill = "white")) + scale_fill_manual(values=cols) + rotate_x_text(45) + guides(fill = guide_legend(nrow = 2))
  ecdf_plot <- ggecdf(plot_df, x = paste(type), color = "dyn") + scale_colour_manual(values=cols) + guides(color = guide_legend(nrow = 2))
  grid.arrange(violin_plot, ecdf_plot, top=paste(timepoint), ncol=2)
}

res = lapply(stages, plot_stage_dynamics, type="peak.width", groups)
res = lapply(stages, plot_stage_dynamics, type="promoter.enrich", groups)
res = lapply(stages, plot_stage_dynamics, type="promoter.mean", groups)
res = lapply(stages, plot_stage_dynamics, type="promoter.max", groups)
