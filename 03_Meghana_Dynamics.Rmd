...
axis_titles = list("peak.width" = "Peak breadth", "promoter.mean" = "Peak mean", "expression.6hpf"="RNA expression at 6hpf [log]", "expression.7hpf"="RNA expression at 7hpf [log]", "expression.8hpf"="RNA expression at 8hpf [log]", "expression.9hpf"="RNA expression at 9hpf [log]")
```
...
plot_balloonplot <- function(remove_ND_NNNN){
  ...
  ggballoonplot(p_mat, fill="value") +
                        theme_light() + labs(x="RNA expression", y=paste0("H3K4me3")) + rotate_x_text(45) +
                        scale_fill_gradientn(colors = my_cols, values=c(0, 0.05, 0.5, 1), breaks=c(0, 2, 20, 50, 100, 200)) + guides(size=guide_legend(title="-log10(p value)"), fill=guide_legend(title="-log10(p value)")) + scale_size(range = c(0, 10), breaks =c(0, 2, 20, 50, 100, 200))
}
...
plot_peak <- function(name, groups, onlypeaks=FALSE, remove_NA=TRUE){
  if(groups=="rna"){
    dyn = rna_dyns
    cols = cols_rnadyns
  } else if(groups=="chip"){
  ...
  if(onlypeaks){stage_raw <- stage_raw[chip_dfs[[paste(name)]]$peak, ]}
  ...
  plot <- ggline(melted, x = "Genomic location", y = "Coverage",
   add = "mean_se", conf.int = TRUE, color="dynamics") + scale_color_manual(values=cols) + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + labs(y=expression(atop(Mean~H3K4me3~ChIP-Seq~intensity,log[2](Coverage + 1))), x = "+/-1kb of TSS") + ggtitle(name) + guides(color = "none")
}
...
plot_stage_dynamics <- function(timepoint, type, groups, onlypeaks=FALSE, legend=FALSE, remove_NA=TRUE, grouped=FALSE){
  if(groups == "rna"){
    cols = cols_rnadyns
    dyn = rna_dyns
  } else if(groups == "chip"){
    #if(type %in% c("peak.width", "promoter.enrich", "promoter.mean", "promoter.max")){onlypeaks=TRUE}
    cols = cols_chipdyns
    dyn = chip_dyns
  }
  label_dyn <- function(gene, dyns){for(i in 1:length(dyns)){if(any(gene %in% dyns[[i]])){return(names(dyns)[i])}}; return(NA)}
  ...
  if(grouped){
    plot_df$dyn_group <- factor(chip_clustering[as.vector(plot_df$dyn)])#levels = c("Kept", "Gained", "Lost", "Recovered","Absent")
...
plot(plot_stage_dynamics("Spermatid", type="cg.density", groups, legend=TRUE))
plot(plot_stage_dynamics("Spermatid", type="promoter.dna.methyl", groups, legend=TRUE))
```
...
### RNA plot
mean_data <- aggregate(. ~  RNA.group, data=heatmap_data[!colnames(heatmap_data) %in% c("Chip.group", "ZGA.expression.9hpf", "ZGA.expression.8hpf", "ZGA.expression.7hpf", "ZGA.expression.6hpf" )], FUN=mean)
...
hm_rna <- Heatmap(scaled_data, 
        ...
        row_split = c("Peak width", "Promoter mean", "DNA Methylation", "Peak width", "Promoter mean", "DNA Methylation", "Peak width", "Promoter mean", "DNA Methylation","Accessibility", "Peak width", "Promoter mean", "CG density"),
        row_title_rot = 0,
...
### Chip plot
mean_data <- aggregate(. ~  Chip.group, data=heatmap_data[!colnames(heatmap_data) %in% c("RNA.group", "ZGA.peak.width", "Pre.ZGA.peak.width", "Spermatid.peak.width", "Sperm.peak.width", "ZGA.promoter.mean", "Pre.ZGA.promoter.mean", "Spermatid.promoter.mean", "Sperm.promoter.mean")], FUN=mean)
...
hm_chip <- Heatmap(scaled_data, 
        ...
        show_row_names = TRUE,
        cluster_rows = clustering,
        row_split = c("DNA methylation", "DNA methylation", "DNA methylation", "Accessibility", "ZGA expression", "ZGA expression", "ZGA expression", "ZGA expression", "CG density"),
        row_title_rot = 0,
